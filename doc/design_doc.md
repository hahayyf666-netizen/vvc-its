...
- 目标时钟频率：500MHz（实际达成 516MHz）
- 每周期 4 点并行计算
- **面积优化：5 个 DSP（行/列引擎复用）**

### 1.2 标准参考

- VVC 标准：ITU-T H.266 / ISO/IEC 23090-3 (JVET-S2001)
- LFNST 矩阵：赛题附件 "Low frequency non.docx"
- 赛题规范：第九届中国研究生创芯大赛 · 华为赛题一

### 1.3 版本历史

| 版本 | 优化 | DSP | WNS | 功耗 |
|------|------|-----|-----|------|
| v1.0 | Baseline | 9 | +0.031ns | 0.318W |
| v1.1 | P2: 行/列引擎复用 | 5 | +0.081ns | 0.214W |
| **v1.2** | P3: BRAM 优化 | **5** | **+0.083ns** | **0.207W** |
...
```
  ...
  it_info ─────────→│  ┌─────────┐                                    │
  it_data_in ──────→│  │ in_mem  │──→ ┌──────────────┐               │
  it_data_addr ────→│  │ (4096)  │    │ Shared Engine│──→ tp_buf     │
                    │  └─────────┘    │ (4 MAC)      │    (4096)     │
                    │                 │ Row/Col 复用  │       │        │
                    │  ┌─────────┐    └──────────────┘       │        │
                    │  │ LFNST   │                    ┌──────┘        │
...
**关键优化 (v1.1/v1.2)**：
- 行变换和列变换串行执行，共享同一组 4 个 MAC
- DSP 从 9 降到 5（-44%）
- 动态功耗从 0.318W 降到 0.207W（-35%）
